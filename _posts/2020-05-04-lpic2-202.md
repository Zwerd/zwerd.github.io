...
When we talk about security in samba world there is two levels, the USER level and the SHARE level, with USER level the client need to supply the username and the password, with SHARE level the client need to supply only password, the server will check if the password is match to one of the users his have on the list and if match was found he will allow the connection for that client.
...
**passdb backend**, this is the method for encription we going to use, in my case is `tdbsam`.
...
Please remember that with samba we just share some file or folder and that can be with security level or not, depending out configuration.
...
There is more way to share files, we can done that by NFS, but unlike samba, NFS is the way we share chank of filesystem instead just files or folders, we saw more or less about NFS in my other post LPIC2 201, the security level on NFS is base only the client itself if he have permition or not to mount the filesystem, it look like iSCSI, that can be use to share SCSI base on the system security.

The presager is working as follow, we setup filesystem to be mount by using NFS, we also setup the client that can have access to that filesystem, the next step is that on the client side we need to mount that NFS filesystem, the network communication is done over rpc bind or port mapper.
...
Ther is more issue that related to security regarding NFS, let's mount that filesystem again and view that issue.

I just change my machine IP to 192.168.43.145 and tried to mount it again, also please note that the username on that machine is zwerd, that zwerd user is also setup on the NFS machine.
...
The security issue came with that userid, if someone change some user to contain the same id of my user, he may get full access to my file, so prevent that can be done by allow only machine that I know for sure that can get access to my files, ot using other security mechanism to prevent that issues.
...
After you done all you need to restart the service, you also can check the `/var/log/syslog` file with contain the logs for the system, in that file we can find that the DHCP is up and running and also what is the count for leases address, if none are lease you will se the following line:
```
dhcp: Wrote 0 leases to leases file.
```
If you what to see the leases address you can check the `/var/lib/dhcp/dhcpd.leases` file, that file will be empty antil you one of the client on that subnet try to request some IP address from our server.
...
Like on cisco routers we can setup on our Linux to by hdcp relay for some subnet that not contain DHCP server, the role of dhcp relay is just to listen to some new leases request and transfer them to the DHCP.
...
Onthe DHCP server we need to add a route to the sparated subnet and also setup new section on `/etc/dhcp/dhcpd.conf` file that contain the range for the new subnet:
```
...
PAM is only provides you as way to do several thing that on your program you didn't need to set, like authentication, there is several program like SSH or telnet or FTP or GNPME, all of then need a way to check the user authentication for knowing if the password is correct or that user allow to use that program and login, instead of develops suck things, they use PAM that provide exactly that.
...
You can see that you have line that specified the `account` and required with pam_nologin.so module, so that line say that need to check is the user account is allow to login by using the `pam_nologin.so` module, please note that the control on that line is `required` which mean that although if that user is not allowed, continue the next checks if any, search down below and you will find the session which is the next check.
...
You can see the password of the administrator and also the guy user, if you use `-w` insead of `-W` then you must type your password on the command line.
...
**Figure 129** Please note that the LDAP password come as the end.
...
In this chapter we going to look how so setup and use email server, we also look at two type of email servers we can use in linux and also two type of protocols in that field.

So let's talk about **postfix** which is mail server that started life at IBM research as an alternative to the widely-used Sendmail program, Sendmailis really vulnerable mail server so this is why it is not in use any more.
...
`relayhost` is empty, so no one can use our serveer as realy server for sending mail and this is way upper you have the smtpd_relay_restrictions to prevent the relay server action.
...
You can see the diffrent between emails that cam to your local users, let's look how it done for root user, I sent mail to the root and specified the localhost for it.
...
**pop3** - in that way we connecting to the outlook service and download the all the mailbox to out machine, in that way we can view email from our local machine although we won't have setup online service just relay on another.
...
The we need to run the following:
```
...
**input** - this chain are apply policy for packet that destinate to out machine, in this chain we can manipulate the packet or drop the packet etc.

**forwarding** - if the packet are not destinate to out machine which mean that out machine need to make a route choice for that packet, in that case the packets are landed on the forwarding chain, and again we can applied policy on that chain to manipulate the decision it must make for route the packet.
...
**DROP** - like ACCEPT this policy is also apply on the chain base on the packet detail, in the DROP the packet will block and the source we never know about that which mean that is just block with no reply to the source.
...
So in my case the ip_forword flag was set so this is way you saw it forwarded on the tcpdump, after I setup that and run tcpdump again with ping from the virtual Ubuntu I get the following results.
...
You can see that I specified the chain and the number line and also the table of nat, without the table the default is filter so you will get the error `iptables: No chain/target/match by that name.` so please not that on PREROUTING you specified the nat table, we also can use `-f` to flash the entire table, now let's say that I want to redirect every client on the local network that trying to connect my Web site that locate on my PC, the WEB site is run on port 80 but my clients didn't know that and they use port 8080, so I need to setup policy rule on the PREROUTING chain to catch every packet that come in and redirect it to port 80, for that I need to setup the following:
```
...
![LPIC2 Post](/assets/images/lpic2/scanporttelnet.png)
...
You can see that I run that locally and it found several port that are open on my computer so I may be want to shut all of the port, so I need to check what process run and use that ports and disable or remove that process.
...
This is the configuration file, you can see that the source can retry to login 5 time only, after that he will banding for a 600 seconds which is 10 minutes, to if I try to login with incorrect password it will kick me out and any new connection from my source will be refuse
...
If you want to get more knowledge that related to penetration testing please check my other post about the [OSCP](http://zwerd.com/2019/09/20/oscp_certificate.html) which is contain several stuff related to this field.
