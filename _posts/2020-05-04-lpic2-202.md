...
**Figure 42** The files in httpd folder.
...
The AuthUserFile used for the direct path to the password file, this is the were the password and usernames stored.
...
This is the same like `service apache2 restart` done, we need to create the secret folder and add some html code that only authorize users can get to.
```
...
The client trying to connect web server by using https, it actually open SSL connection, at first there is the 3 way hand shake in TCP and then he try to connect using https, the server will give him his public key that sign by some root authority that browser on the client trust, then the server send trafice down the client that incripted by the server privet key, the browser on the client decript that by using the public key, all of that is for deny Man In The Middle attack that act like he is the server and give the client some sert, because the client has the root authority that he trust so in that case we will continue the session only with server that have certificate that sign with the root authority.
